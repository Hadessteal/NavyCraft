...
public class CraftMover {
	...
	public HashMap<Player, Location> playerTeleports = new HashMap<Player, Location>();
	
	public ArrayList<Block> initWeaponDispensers = new ArrayList<Block>();
				...
				if(inventory != null) {
					...
					for(int slot = 0; slot < inventory.getSize() && slot<27; slot++) {
						if(inventory.getItem(slot) != null && inventory.getItem(slot).getTypeId() != 0) {
							...
							NavyCraft.instance.DebugMessage("Inventory has " + inventory.getItem(slot).getAmount() + 
	...
	public void restoreComplexBlocks(int dx, int dy, int dz) {
		...
		for (DataBlock complexBlock : craft.complexBlocks) {
			...
			if ((complexBlock.id == 63 || complexBlock.id == 68) ) {
				...
				/*if( complexBlock.signLines[0].equalsIgnoreCase("periscope") )
				{
					craft.periscopeLoc = theBlock.getLocation();
				}*/
			...
			if (inventory != null) {
				for(int slot = 0; slot < inventory.getSize() && slot < 27; slot++) {
					if(complexBlock.items[slot] != null && complexBlock.items[slot].getTypeId() != 0) {
						...
						NavyCraft.instance.DebugMessage("Moving " + complexBlock.items[slot].getAmount() + 
...
/*
		...
		}else
		{
			...
			while (!craft.canMove(dx, dy, dz)) 
			{
				...
				if (craft.speed == 1) 
				{
					...
					//craft.player.sendMessage(ChatColor.RED + "the " + craft.name + " won't go any further");
					if( craft.possibleCollision && !checkProtectedRegion(craft.collisionLoc) && !MoveCraft.checkSpawnRegion(craft.collisionLoc) && !craft.type.canFly)
					{
						if( craft.driverName != null )
						{
							Player p = plugin.getServer().getPlayer(craft.driverName);
							if( p != null )
								p.sendMessage(ChatColor.RED + "Collision!!! PANIC!!");
						}
							
						doCollide = true;
						craft.possibleCollision = false;
						break;
					}else if( MoveCraft.checkSpawnRegion(craft.collisionLoc) )
					{
						if( craft.driverName != null )
						{
							Player p = plugin.getServer().getPlayer(craft.driverName);
				...
				for (int x = 0; x < 9; x++) 
				{
					for( int y = 0; y < 9; y++ )
					{
					    for (int z = 0; z < 9; z++) 
					    {
					    	...
					    	Block theBlock = colBlock.getRelative(x - 4, y - 4, z - 4);
					    	...
					    	if( breakChance == -1 )
					    	{
								//CraftWorld cWorld = (CraftWorld) colBlock.getWorld();
					    		...
					    		//theBlock.getWorld().
					    		TNTPrimed tnt = (TNTPrimed)theBlock.getWorld().spawnEntity(new Location(theBlock.getWorld(), theBlock.getX(), theBlock.getY(), theBlock.getZ()), org.bukkit.entity.EntityType.PRIMED_TNT);
		...
		for (int x = 0; x < craft.sizeX; x++) {
			for (int z = 0; z < craft.sizeZ; z++) {
				for (int y = 0; y < craft.sizeY; y++) {
					...
					if (x - craft.dx >= 0 && y - craft.dy >= 0 && z - craft.dz >= 0
							&& x - craft.dx < craft.sizeX && y - craft.dy < craft.sizeY
							&& z - craft.dz < craft.sizeZ) {

						// after moving, this location is not a craft block anymore
						if (craft.matrix[x - craft.dx][y - craft.dy][z - craft.dz] == -1
								|| BlocksInfo.needsSupport(craft.matrix[x - craft.dx][y - craft.dy][z - craft.dz])) {
					...
					if (!BlocksInfo.needsSupport(blockId)) {
						...
						if (x + craft.dx >= 0 && y + craft.dy >= 0 && z + craft.dz >= 0
							...
							if (craft.matrix[x][y][z] != craft.matrix[x + craft.dx][y + craft.dy][z + craft.dz]) {
								...
								{
									if( blockId != 8 && blockId != 9 )//&& blockId != 0 )
						...
						// outside of the previous bounding box
							...
							if( blockId != 8 && blockId != 9 && blockId != 0 )
								...
								//if( y <= craft.waterLevel)////block.getY() >= 63 )
								if( block.getY() >= 63 )
									setBlock(0, innerBlock);
								else
									setBlock(8, innerBlock);
				...
				for (int y = 0; y < craft.sizeY; y++) {
					...
					/*if (x - craft.dx >= 0 && y - craft.dy >= 0 && z - craft.dz >= 0
						...
						if (craft.matrix[x][y ][z] == -1
								|| BlocksInfo.needsSupport(craft.matrix[x][y][z])) {
							//if (y > craft.waterLevel || y > 63 || !(craft.type.canNavigate || craft.type.canDive)) {
								//|| craft.matrix [ x - dx ] [ y - dy ] [ z - dz ] == 0)
								//if( !(y > 63 && blockId == 8 && blockId == 9) )
							if ( block.getY() >= 63 || (!(craft.type.canNavigate || craft.type.canDive) && !craft.sinking) )
	...
	public void move3(boolean scheduledMove)
	{
		...
		for(Entity e : craft.checkEntities) {
				...
				//{
					...
					//}
				//}
				if(NavyCraft.instance.ConfigSetting("TryNudge").equalsIgnoreCase("true") &&
						(craft.type.listenMovement == false || (pl != null && e != pl)) ) {
	...
	/*	if( !craft.checkedChunks.isEmpty() )
    	{  	
				CraftPlayer cp;
				EntityPlayer ep;
				for( Chunk checkChunk : craft.checkedChunks )
				{
					...
					for(Player p : craft.world.getPlayers())
					{
						cp = (CraftPlayer)p;
						ep = cp.getHandle();
						if ( !ep.chunkCoordIntPairQueue.contains(new ChunkCoordIntPair(checkChunk.getX(), checkChunk.getZ())) )
	...
	public void delayed_move_collide(int dx, int dy, int dz) {
		...
		if( craft.type.canNavigate || craft.type.canDive )
		{
				...
				for( Entity e : craft.checkEntities )
				{
					if( e instanceof Player )
					{
						...
						if( craft.submergedMode )
							p.playSound(p.getLocation(), Sound.SPLASH2, 1.0f, 0.6f);
						else
							p.playSound(p.getLocation(), Sound.SPLASH, 0.5f + 0.5f*(float)craft.speed/12.0f, 0.6f);
					}
				}
			}
			/*for( String s : craft.crewNames )
			{
				...
				if( p != null && craft.isNameOnBoard.get(s) )
				{
					if( craft.submergedMode )
						p.playSound(p.getLocation(), Sound.SPLASH2, 1.0f, 0.6f);
					else
	...
	public void move(int dx, int dy, int dz, boolean checkStructure) {
		...
		for(Entity e : checkEntities) {
			//if(craft.isOnCraft(e, false)) {
				if(NavyCraft.instance.ConfigSetting("TryNudge").equalsIgnoreCase("true") &&
						(craft.type.listenMovement == false || (p != null && e != p)) ) {
	...
	public void structureUpdate(Player causer, boolean scheduled) {
		...
		for (int y = 0; y < craft.sizeY; y++) {
				...
				for (int z = 0; z < craft.sizeZ; z++) 
				{
					...
					if( craft.type.canNavigate || craft.type.canDive )
					{
						if( newBlock.getY() < 63 && !(blockId >= 8 && blockId <= 11) )
						{
							...
							}else if( blockId == 121 )
							{
								...
								if( disp < 1 )
									disp = 1;
								disp = (float)(craft.ballastAirPercent - 100) * .01f * disp;
								displacement += disp;
					...
					if( craftBlockId == -1 || craftBlockId == 0 || (craftBlockId >= 8 && craftBlockId <= 11) )
					{
						...
						else if( blockId != 0 && !(blockId >= 8 && blockId <= 11) && blockId != 79 )
						{
							...
							{
								Player p = plugin.getServer().getPlayer(craft.driverName);
								//if( p != null && Craft.getOtherCraft(craft, p, newBlock.getLocation().getBlockX(), newBlock.getLocation().getBlockY(), newBlock.getLocation().getBlockZ()) == null)
					...
					}else
					{
						...
						{
							//if( blockId == 42 || blockId == 41 || blockId == 1 || blockId == 98 || blockId == 5 || blockId == 69 || blockId == 77 )
						...
						if (blockId == 0 || (blockId >= 8 && blockId <= 11)) {
							...
							if( (blockId == 0 && (craftBlockId == 50 || craftBlockId == 75 || craftBlockId == 76 || craftBlockId == 65 || craftBlockId == 68 || craftBlockId == 63 || craftBlockId == 69 || craftBlockId == 77 || craftBlockId == 70 || craftBlockId == 72 || craftBlockId == 55 || craftBlockId == 143 || craftBlockId == 64 || craftBlockId == 71)
									)&& (Craft.getAttachedBlock(newBlock, craftBlockId, data) != null && Craft.getAttachedBlock(newBlock, craftBlockId, data).getTypeId() != 0 && !(Craft.getAttachedBlock(newBlock, craftBlockId, data).getTypeId() >=8 && Craft.getAttachedBlock(newBlock, craftBlockId, data).getTypeId() <=11)))
							{
							...
							{
								//if( !(craftBlockId == 50 || craftBlockId == 75 || craftBlockId == 76 || craftBlockId == 65 || craftBlockId == 68 || craftBlockId == 69 || craftBlockId == 77 || craftBlockId == 70 || craftBlockId == 72 || craftBlockId == 55 || craftBlockId == 143) )
								//{
									...
									NavyCraft.instance.DebugMessage("Removing a block of type " + craftBlockId + 
			...
			/*
			if( craft.buoyancy == 0 && (craft.type.canNavigate || craft.type.canDive) && displacement > craft.weight * craft.weightMult )
			{
				if( lastY < 62 )
				{
					if( craft.type.canDive && displacement > craft.weight*craft.weightMult )
					{
						if( displacement > craft.weight*craft.weightMult*1.3f )
							craft.buoyancy = 2;
						else
							craft.buoyancy = 1;
					}
			...
			if( scheduled || craft.gear == 3 )
			{
				...
				if( craft.buoyFloodTicker%2 == 0 )
				{
					for( Entity e : craft.checkEntities )
					{
						...
						{
							Player p = (Player)e;
							p.playSound(p.getLocation(), Sound.SWIM, 0.6f, 0.4f + 0.2f * (float)craft.speed / 12.0f);
						}
					}
					/*for( String s : craft.crewNames )
					{
		...
		/*if( craft.currentEngineCount == 0 && craft.useEngine )
		{
			craft.helmDestroyed = true;
			craft.setSpeed = 0;
		}
		
		if( !craft.engineIDLocs.isEmpty() && !craft.useEngine )
		{
			craft.useEngine=true;
		}*/
		...
		if( craft.blockCount < craft.lastBlockCount )
		{
			if( causer != null )
			{
				...
				//plugin.getServer().broadcastMessage("damage caused by " + causer.getName() + " " + (craft.lastBlockCount - craft.blockCount) );
			}else
			{
				...
				if( craft.lastCauser != null )
				{
					...
					//plugin.getServer().broadcastMessage("damage caused by (guess) " + craft.lastCauser.getName() + " " + (craft.lastBlockCount - craft.blockCount) );
				}else
				{
					craft.uncreditedDamage += craft.lastBlockCount - craft.blockCount;
					//plugin.getServer().broadcastMessage("uncredited damage " + (craft.lastBlockCount - craft.blockCount) );
				}
			}
	...
	public void updateBuoyancy(float displacement)
	{
		...
		/*if( (scheduled || craft.gear == 3) && ((craft.buoyFloodTicker == 3 && Math.abs((displacement-craft.weight*craft.weightMult)/(craft.weight*craft.weightMult)) > 0.3f ) || craft.buoyFloodTicker == 7))
		{
			//buoyancy
			if( (craft.type.canNavigate || craft.type.canDive) && craft.buoyancy != 0 )
			{
	...
	public void signUpdates(Block newBlock)
	{
					...
					if( p.periscopeID == periscopeId )
					{
						...
						if( !p.destroyed )
						{
							for( int k=1; k<=20; k++)
							{
								...
								for( int i=-3; i<=3; i++)
								{
									if(newBlock.getRelative(BlockFace.UP, k).getRelative(BlockFace.EAST,i).getTypeId() == 41
											&& newBlock.getRelative(BlockFace.UP, k+1).getRelative(BlockFace.EAST,i).getTypeId() == 113)
									{
										...
										if( p.raised )
										{
											if( newBlock.getRelative(BlockFace.UP, k+2).getRelative(BlockFace.EAST,i).getTypeId() == 113
													&& newBlock.getRelative(BlockFace.UP, k+3).getRelative(BlockFace.EAST,i).getTypeId() == 113
													&& newBlock.getRelative(BlockFace.UP, k+4).getRelative(BlockFace.EAST,i).getTypeId() == 113
													&& newBlock.getRelative(BlockFace.UP, k+5).getRelative(BlockFace.EAST,i).getTypeId() == 113)
											{
												if( !signUp )
												{
								...
								if( !found )
								{
									for( int j=-3; j<=3; j++)
									{
										if(newBlock.getRelative(BlockFace.UP, k).getRelative(BlockFace.NORTH,j).getTypeId() == 41
												&& newBlock.getRelative(BlockFace.UP, k+1).getRelative(BlockFace.NORTH,j).getTypeId() == 113)
										{
											...
											if( p.raised )
											{
												if( newBlock.getRelative(BlockFace.UP, k+2).getRelative(BlockFace.NORTH,j).getTypeId() == 113
														&& newBlock.getRelative(BlockFace.UP, k+3).getRelative(BlockFace.NORTH,j).getTypeId() == 113
														&& newBlock.getRelative(BlockFace.UP, k+4).getRelative(BlockFace.NORTH,j).getTypeId() == 113
														&& newBlock.getRelative(BlockFace.UP, k+5).getRelative(BlockFace.NORTH,j).getTypeId() == 113)
												{
													if( !signUp )
													{
														sign.setLine(1, "||"+periscopeId+"||");
		...
		}else if( craftTypeName.equalsIgnoreCase("engine") ) 
		{
			...
			if( !engineTypeStr.isEmpty() )
			{
				...
				if( engineTypeStr.equalsIgnoreCase("Diesel 2") )
					engType = 2;
				if( engineTypeStr.equalsIgnoreCase("Boiler 1") ) //max spd = 6
					engType = 3;
				...
				if( craft.tubeFiringMode.containsKey(tubeNum) )
				{
					...
					if( mode == -3 )
					{
						...
						switch(display)
						{
							/*case 0:
								line3 = "RUD=";
								if( craft.tubeFiringRudder.get(tubeNum) )
									line3 += "AUTO";
								else
					...
					else if( mode == -2 )
					{
						...
						{
							/*case 0:
								line3 = "HDG=" + craft.tubeFiringHeading.get(tubeNum);
								break;*/
							case 0:
		...
		}else if( craftTypeName.equalsIgnoreCase("radar") ) 
		{
			if( craft.radarOn && !craft.sinking && craft.maxY > 63)
			{
				...
				HashMap<Integer, String> bearings = new HashMap<Integer, String>(); 
				for( Craft c: Craft.craftList )
				{
					if( c != craft && c.world == craft.world && !c.sinking && c.maxY > 65 )
					{
						float xDist = ((float)c.minX + (float)c.sizeX/2.0f) - ((float)craft.minX + (float)craft.sizeX/2.0f);
						float zDist = ((float)c.minZ + (float)c.sizeZ/2.0f) - ((float)craft.minZ + (float)craft.sizeZ/2.0f);
		...
		}else if( craftTypeName.equalsIgnoreCase("sonar") ) 
		{
			if( craft.sonarOn && !craft.sinking && (craft.type.canNavigate || craft.type.canDive) )
			{
				...
				{
					//craft.world.playSound(new Location(craft.world, craft.minX+craft.sizeX/2, craft.minY+craft.sizeY/2, craft.minZ+craft.sizeZ/2), Sound.BREATH, 3.0f, 0.70f);
					craft.lastSonarPulse = System.currentTimeMillis();
					//if( craft.captainName != null )
						//plugin.getServer().getPlayer(craft.captainName).playNote(newBlock.getRelative(BlockFace.DOWN, 2).getLocation(), Instrument.PIANO, Note.natural(1, Tone.E));
					HashMap<Integer, String> bearings = new HashMap<Integer, String>(); 
					for( Craft c: Craft.craftList )
					{
						if( c != craft && c.world == craft.world && !c.sinking && (c.type.canNavigate || c.type.canDive) )
						{
							...
							if( dist < 200 )
							{
								...
								if( bearings.containsKey(relBearing) )
								{
									...
									bearings.put(relBearing, distanceStr);
								//c.world.playSound(new Location(c.world, c.minX+c.sizeX/2, c.minY+c.sizeY/2, c.minZ+c.sizeZ/2), Sound.BREATH, 3.0f, 0.70f);
							}
						}
					}
		...
		}else if( craftTypeName.equalsIgnoreCase("detector") ) 
		{
			if( !craft.sinking && craft.maxY > 63 )
			{
				...
				for( Craft c: Craft.craftList )
				{
					...
					if( c != craft && c.world == craft.world && 
							( radarEcho||radioEcho ))
					{
						float xDist = ((float)c.minX+(float)c.sizeX/2.0f) - ((float)craft.minX+(float)craft.sizeX/2.0f);
						float zDist = ((float)c.minZ+(float)c.sizeZ/2.0f) - ((float)craft.minZ+(float)craft.sizeZ/2.0f);
		...
		}else if( craftTypeName.equalsIgnoreCase("hydrophone") ) 
		{
			if( !craft.sinking && (craft.type.canNavigate || craft.type.canDive))
			{
				...
				{
					if( c != craft && c.world == craft.world && 
							( ((System.currentTimeMillis() - c.lastSonarPulse)/1000 < 10)
							|| ((c.type.canNavigate || c.type.canDive) && c.enginesOn))	) 
					{
						float xDist = ((float)c.minX+(float)c.sizeX/2.0f) - ((float)craft.minX+(float)craft.sizeX/2.0f);
						float zDist = ((float)c.minZ+(float)c.sizeZ/2.0f) - ((float)craft.minZ+(float)craft.sizeZ/2.0f);
						float dist = (float)Math.sqrt(xDist*xDist + zDist*zDist);
		...
		}else if( craftTypeName.equalsIgnoreCase("passivesonar") ) 
		{
			if( !craft.sinking && (craft.type.canNavigate || craft.type.canDive))
			{
				HashMap<Craft, Integer> bearings = new HashMap<Craft, Integer>(); 
				HashMap<Craft, Float> strengths = new HashMap<Craft, Float>();
				HashMap<Integer, Craft> displayBearingsStrong = new HashMap<Integer, Craft>(); 
				HashMap<Integer, Craft> displayBearingsMedium = new HashMap<Integer, Craft>(); 
				HashMap<Integer, Craft> displayBearingsWeak = new HashMap<Integer, Craft>(); 
				for( Craft c: Craft.craftList )
				{
					if( c != craft && c.world == craft.world && 
							(  ((c.type.canNavigate || c.type.canDive) && c.enginesOn)) ) 
		...
		else if( craftTypeName.equalsIgnoreCase("hfsonar") ) 
		{
			if( !craft.sinking && (craft.type.canNavigate || craft.type.canDive) && craft.hfOn )
			{
				...
				case (byte) 0x2://n
					...
					horizCenter = (int)(craft.minX + (float)craft.sizeX/2.0f);
					vertCenter = (int)(craft.minY + (float)craft.sizeY/2.0f);
					lrDir = -1;
					lrX = true;
					start = craft.maxZ;
					forDir = 1;
				...
				case (byte) 0x4://w
					...
					horizCenter = (int)(craft.minZ + (float)craft.sizeZ/2.0f);
					vertCenter = (int)(craft.minY + (float)craft.sizeY/2.0f);
					lrDir = 1;
					start = craft.maxX;
					forDir = 1;
	...
	public void teleportPlayer(Entity p, int dx, int dy, int dz) {
		/*if( p.getType() == org.bukkit.entity.EntityType.DROPPED_ITEM )
		{
			p.
		}*/
	...
	/*public boolean AsyncMove(int dx, int dy, int dz) {
		...
		Runnable r = new Runnable() {
			public void run() {
				...
				move(changeX, changeY, changeZ, false, false);
			}
		};
    ...
    public boolean checkSink()
    {
    	...
    	if( (float)solidCount/( (float)craft.sizeX * (float)craft.sizeZ ) >= 0.6f )
    	{
...
//////////////////////////******************?////////////////
    ...
    public void moveUpdate()
    {
		...
		/*}else if( craft.moveTicker == 1 )
		{
			move1();
		}else if( craft.moveTicker == 2 )
		{
			move2();
		}else if( craft.moveTicker == 3 )
		{
			move3();
		}*/
    }
	...
	public void cruisingThread(final Craft fcraft){
    	//final int taskNum;
    	//int taskNum = plugin.getServer().getScheduler().scheduleAsyncDelayedTask(plugin, new Runnable(){
		stopSound=false;
		
		if( fcraft.engineIDLocs.isEmpty() && !fcraft.useEngine )
		{
			if( fcraft.driverName != null )
			{
				Player p = plugin.getServer().getPlayer(craft.driverName);
				if( p != null )
				{
					if( fcraft.type.canFly )
						p.sendMessage("Warning: No engines detected! Engaging rubber band power.");
		    		else if( fcraft.type.isTerrestrial )
		    			p.sendMessage("Warning: No engines detected! Engaging Flintsone power.");
		    		else
						p.sendMessage("Warning: No engines detected! Engaging oars.");
				}
			}
			soundThread(fcraft, -1);
		}else
		{
			for(int id: fcraft.engineIDLocs.keySet())
			{
				craft.engineIDOn.put(id, true);
				soundThread(fcraft, id);
			}
		}
    	...
    	@Override
			...
			try{
				/*if( fcraft.driverName != null )
				{
					Player p = plugin.getServer().getPlayer(fcraft.driverName);
					if( p != null )
						p.sendMessage("Starting Engines.");
				}
				if( fcraft.driverName != fcraft.captainName && fcraft.captainName != null )
				{
					Player p = plugin.getServer().getPlayer(fcraft.captainName);
					if( p != null )
						p.sendMessage("Starting Engines.");
				}*//*
				int noCaptain=0;
				int stuckAutoTimer=0;
				
				
				
				while( fcraft != null && (fcraft.speed > 0 || fcraft.setSpeed > 0 ) && !fcraft.sinking && noCaptain < 200 && stuckAutoTimer < 20 )
				{
					if( Math.abs(fcraft.gear) == 1 || fcraft.gear == 0 )
		    		{
		    			if( fcraft.type.canFly )
		    				sleepTime = 6000;
		    			else
		    		...
		    		else if( Math.abs(fcraft.gear) == 2 )
		    		{
		    			if( fcraft.type.canFly )
		    				sleepTime = 4500;
		        		else
		    		...
		    		else if( Math.abs(fcraft.gear) == 3 )
		    		{
		    			if( fcraft.type.canFly )
		    				sleepTime = 3000;
		        		else
					...
					sleep(sleepTime);
					cruiseUpdate();
					if( fcraft.captainName == null && !fcraft.type.canZamboni && !fcraft.isAutoCraft )
					{
						noCaptain++;
					}else if( noCaptain > 0 )
					{
					...
					if( fcraft.isAutoCraft && fcraft.speed == 0 )
					{
						stuckAutoTimer++;
					}else if( fcraft.isAutoCraft && stuckAutoTimer > 0 )
					{
						stuckAutoTimer = 0;
					}
				}
				...
				if( stuckAutoTimer == 20 )
				{
					fcraft.Destroy();
					plugin.getServer().broadcastMessage(ChatColor.YELLOW + "ENEMY MERCHANT - Mission FAILED! Merchant reached its destination.");
				}
				if( fcraft.driverName != null )
				{
					Player p = plugin.getServer().getPlayer(fcraft.driverName);
				...
				if( fcraft.captainName != null && fcraft.driverName != fcraft.captainName )
				{
					Player p = plugin.getServer().getPlayer(fcraft.captainName);
					if( p != null )
						p.sendMessage("Engines Stopped.");
	...
	public void cruiseUpdate()
    {
	  ...
	  //  @Override
		    ...
		    {*/
		    				...
		    				if( ((craft.type.canNavigate&&!craft.type.isTerrestrial) && (engVehicleType == 0||engVehicleType == 1)) || (craft.type.canDive && (engVehicleType == 0||engVehicleType == 1||engType==9)) || (craft.type.canFly && engVehicleType == 2 && craft.driverName != null) || (craft.type.isTerrestrial && engVehicleType == 3) )
		    				{
				    			if( craft.engineIDTypes.get(i) == engType )
				    			{
				    				//if( craft.submergedMode && engType != 9 && engType != 1)
				    					//actualSetSpeed += (((float)craft.setSpeed/(float)craft.type.maxEngineSpeed)/2.0f)*maxTypeSpd*powerRating*powerRating/(float)craft.blockCount/(float)craft.blockCount;
				    				//else
				    				float craftWeight;
				    				...
				    				if( craft.submergedMode && engType != 9 && engType != 1)
				    					//totalActualSetSpeed += (float)((((float)craft.setSpeed/(float)craft.type.maxEngineSpeed)*maxTypeSpd*powerRating*powerRating/craftWeight/craftWeight)/2);
				    					totalActualSetSpeed += (float)((maxTypeSpd*powerRating*powerRating/craftWeight/craftWeight)/2);
				    				else
				    					//totalActualSetSpeed += (float)((((float)craft.setSpeed/(float)craft.type.maxEngineSpeed)*maxTypeSpd*powerRating*powerRating/craftWeight/craftWeight));
				    					totalActualSetSpeed += (float)((maxTypeSpd*powerRating*powerRating/craftWeight/craftWeight));
				    			}
				    			
				    			/*if( craft.submergedMode && engType != 9 && engType != 1 && totalActualSetSpeed > maxTypeSpd/2)
				    			{
				    				totalActualSetSpeed = maxTypeSpd/2;
				    			}*/
				...
				if( p != null )
				{
					p.sendMessage(ChatColor.WHITE + "actualSetSpeed=" + actualSetSpeed);
					p.sendMessage(ChatColor.WHITE + "setSpeed=" + craft.setSpeed);
					p.sendMessage(ChatColor.WHITE + "maxEngineSpeed=" + craft.type.maxEngineSpeed);
				}*/
		    	...
		    	if( craft.isAutoCraft )
		    	{
		    		/*if( craft.rudder == -1 && craft.minZ > 400 )
		    		{
		    			craft.rudderChange(null, 0, false);
		    		}
		    		if( craft.rudder == 1 && craft.minZ < -800 )
		    		{
		    			craft.rudderChange(null, 0, false);
		    		}
		    		if( craft.minX > 1300 )
	...
	public void sinkBroadcast()
	{
		...
		{
			HashMap<Craft, Integer> craftDamagers = new HashMap<Craft, Integer>(); 
			HashMap<Player, Integer> uncrewedPlayers = new HashMap<Player, Integer>(); 
			int totalDamage=0;
			...
			if( craft.isAutoCraft )
			{
				...
				if(topPlayer != null )
				{
					if( craft.crewHistory.contains(topPlayer.getName()) )
					{
						return;
					}
				...
				}else if( topCraft != null && topCraft != craft )
				{
					...
					{
						if( craft.crewHistory.contains(s) )
							return;
					}
			...
			if( craft.world.getName().equalsIgnoreCase("warworld2") && NavyCraft.battleMode > 0 )
			{
				...
				if( topPlayer != null )
				{
					if( (!NavyCraft.redPlayers.contains(topPlayer.getName()) && !NavyCraft.bluePlayers.contains(topPlayer.getName()))
							|| (NavyCraft.redPlayers.contains(topPlayer.getName()) && !craft.blueTeam)
							|| (NavyCraft.bluePlayers.contains(topPlayer.getName()) && !craft.redTeam) )
					{
						return;
					}
					int newExp = craft.blockCountStart;
				...
				{
					if( (!NavyCraft.redPlayers.contains(topCraft.captainName) && !NavyCraft.bluePlayers.contains(topCraft.captainName))
							|| (NavyCraft.redPlayers.contains(topCraft.captainName) && !craft.blueTeam)
							|| (NavyCraft.bluePlayers.contains(topCraft.captainName) && !craft.redTeam) )
					{
						return;
					}
					int newExp = craft.blockCountStart;
	...
	@Override
		...
		try{
			...
			while( !stopSound && !c.isDestroying && !c.sinking ) 
			{
				...
				if( i%4 == 0 )
				{
					sleep=(2000-(int)(1000*(float)(c.setSpeed*Math.abs(c.gear))/(float)(c.type.maxEngineSpeed*c.type.maxForwardGear)))/4;
					if( c.type.canFly )
						sleep=(1000-(int)(500*(float)(c.setSpeed*Math.abs(c.gear))/(float)(c.type.maxEngineSpeed*c.type.maxForwardGear)))/4;
				}
				sleep(sleep);
				if( NavyCraft.shutDown )
					return;
	...
	public void soundUpdate(final Craft c, final int engineIndex, final int i)
    {
	  ...
	  //  @Override
		    public void run()
		    {
				...
				if( c.type.canFly )
				{
					volume=2.0f*(float)(c.setSpeed*Math.abs(c.gear))/(float)(c.type.maxEngineSpeed*c.type.maxForwardGear) + 1.0f;
				...
				if( c.setSpeed > 0 )
				{
					if( noEngineLoc != null )
					{
						float pitch=(float)c.setSpeed/(float)c.type.maxEngineSpeed + 0.5f;
						if( i%5 == 0 )
						{
							
							if( craft.type.canFly )
								cw.playSound(noEngineLoc, Sound.ARROW_HIT, volume, pitch);
				    		else if( craft.type.isTerrestrial )
				    			cw.playSound(noEngineLoc, Sound.STEP_GRAVEL, volume, pitch);
				    		else
				    			cw.playSound(noEngineLoc, Sound.WATER, volume, pitch);
						}
					}else if( c.submergedMode )
					{
						float pitch=(float)c.setSpeed/(float)c.type.maxEngineSpeed + 0.5f;
						if( i%2 == 0 )
							cw.playSound(cLoc, Sound.WATER, volume*2, pitch);
					}else
					{
						float pitch1=1.0f;
						float pitch2=2.0f;
						...
						if( engType == 0 ) //Diesel 1
		    			{
							...
							pitch2=0.5f;
							if( (i)%4 == 0 )
								cw.playSound(cLoc, Sound.PISTON_EXTEND, volume, pitch1);
							else if( (i)%4 == 1 )
								cw.playSound(cLoc, Sound.PISTON_EXTEND, volume, pitch2);
							else if( (i)%4 == 2 )
								cw.playSound(cLoc, Sound.PISTON_RETRACT, volume, pitch1);
							else if( (i)%4 == 3 )
								cw.playSound(cLoc, Sound.PISTON_RETRACT, volume, pitch2);
		    			}
						if( engType == 1 ) //Motor 1
		    			{
							float pitch=(float)c.setSpeed/(float)c.type.maxEngineSpeed + 0.5f;
							if( i%2 == 0 )
								cw.playSound(cLoc, Sound.WATER, volume, pitch);
		    			}
		    			if( engType == 2 ) //Diesel 2
		    			{
		    				pitch1=0.8f;
	...
	public void teleportUpdate()
    {
	  ...
	  //  @Override
		    ...
		    {	
				...
				/*for (Chunk checkChunk : craft.checkedChunks) 
				{
		    		CraftChunk cc = (CraftChunk) checkChunk;
		    		net.minecraft.server.v1_8_R3.Chunk netChunk = cc.getHandle();
		    		netChunk.initLighting();
		    		
		    		PacketPlayOutMapChunk packet = new PacketPlayOutMapChunk(netChunk, true, 0);
		            for(Player p: craft.world.getPlayers())
				    	...
				    	//manager.updateEntity(ent, craft.world.getPlayers());
				    	 ...
				    	 //{
				    		// ce.getNetworkController().makeVisible(tracker);
				    		 //ce.getNetworkController().addViewer(tracker);
				    		 //plugin.getServer().broadcastMessage("trying to update-" + ent.getName() + " for-" + tracker.getName());
				    		 //manager.updateEntity(ent, craft.world.getPlayers());
				    		 
				    	 //}
				    	
				    	/*
				            // The update name packet - you may also want to check whether or not the skeleton needs to 
				            // be updated
				            PacketContainer updateMetadata = manager.createPacket(Packets.Server.ENTITY_METADATA);
				            WrappedDataWatcher watcher = new WrappedDataWatcher();
				            
				            //watcher.setObject(CUSTOM_NAME_INDEX, ((LivingEntity) ent).getCustomName());
				            //watcher.setObject(6, (byte)1); // Display name plate
				            ...
				            for (Player tracker : manager.getEntityTrackers(ent)) {
				                // This will be intercepted by our listener
